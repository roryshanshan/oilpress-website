...
/* Phong cách phần giới thiệu công ty */
.company-intro {
  display: flex;
  gap: 2rem;
  margin: 2rem 0;
...
/* Phong cách phần ưu điểm sản phẩm */
.advantages-grid {
  display: grid;
...
/* Phong cách phần nguyên liệu phù hợp */
.materials-section {
  margin: 2rem 0;
}
.materials-grid {
...
/* Phong cách phần dịch vụ kỹ thuật */
.services-section {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));
  gap: 1.5rem;
...
/* Phong cách phần liên hệ chúng tôi */
.contact-section {
  display: flex;
  gap: 2rem;
  margin: 2rem 0;
...
/* Điều chỉnh phản hồi (responsive) */
